...
for i in range(100):
    ...
    d[0][0].save(f"saved_brains_8/gen_{i}")

    print(f"gen {i} best score: {d[0][1]}\t|\tgen {i} best {sqrt_num} scores mean: {sum(list(map(lambda p: p[1], d))[:sqrt_num])/sqrt_num}\t|\tgen {i} mean: {sum(list(map(lambda p: p[1], d)))/len(d)}")
    print(f"gen {i} best score: {d[0][1]}\t|\tgen {i} best {sqrt_num} scores mean: {sum(list(map(lambda p: p[1], d))[:sqrt_num])/sqrt_num}\t|\tgen {i} mean: {sum(list(map(lambda p: p[1], d)))/len(d)}", file=f)
    
    brains = [b.pso(d[0][0]) for b in brains]
